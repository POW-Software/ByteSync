...
public static class IpAddressExtractor
{
    public static string ExtractIpAddress(this HttpRequestData req)
    {
        ...
        if (headerDictionary.TryGetValue("x-forwarded-for", out var xForwardedFors))
        {
            var ipSegment = xForwardedFors.FirstOrDefault()?.Split(',')[0]?.Trim();
            if (!string.IsNullOrEmpty(ipSegment))
            {
                var ipAddress = DoExtractIpAddress(ipSegment);
                if (ipAddress != null)
                {
                    return ipAddress;
                }
        ...
        if (headerDictionary.TryGetValue("host", out var hosts))
        {
    ...
    private static string? DoExtractIpAddress(string ipSegment)
    {

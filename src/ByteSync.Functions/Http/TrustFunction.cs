...
public class TrustFunction
{
        ...
        FunctionContext executionContext)
    {
        ...
        Console.WriteLine($"[PROTOCOL_VERSION_DEBUG] StartTrustCheck - Raw request body: {requestBody}");
        
        req.Body.Position = 0;
        var parameters = await FunctionHelper.DeserializeRequestBody<TrustCheckParameters>(req);
        
        Console.WriteLine(
            $"[PROTOCOL_VERSION_DEBUG] StartTrustCheck - Deserialized: SessionId={parameters.SessionId}, ProtocolVersion={parameters.ProtocolVersion}, PublicKeyInfo.ProtocolVersion={parameters.PublicKeyInfo.ProtocolVersion}, ClientId={client.ClientInstanceId}");
        
        var request = new StartTrustCheckRequest(parameters, client);
        var result = await _mediator.Send(request);
        
        Console.WriteLine(
            $"[PROTOCOL_VERSION_DEBUG] StartTrustCheck - Result: IsOK={result.IsOK}, IsProtocolVersionIncompatible={result.IsProtocolVersionIncompatible}");
        ...
        FunctionContext executionContext)
    {
        ...
        using var reader = new StreamReader(req.Body);
        var requestBody = await reader.ReadToEndAsync();
        Console.WriteLine($"[PROTOCOL_VERSION_DEBUG] GiveMemberPublicKeyCheckData - Raw request body: {requestBody}");
        
        req.Body.Position = 0;
        var parameters = await FunctionHelper.DeserializeRequestBody<GiveMemberPublicKeyCheckDataParameters>(req);
        
        Console.WriteLine(
            $"[PROTOCOL_VERSION_DEBUG] GiveMemberPublicKeyCheckData - Deserialized: SessionId={parameters.SessionId}, ClientInstanceId={parameters.ClientInstanceId}, PublicKeyCheckData.ProtocolVersion={parameters.PublicKeyCheckData.ProtocolVersion}, PublicKeyCheckData.IssuerPublicKeyInfo.ProtocolVersion={parameters.PublicKeyCheckData.IssuerPublicKeyInfo.ProtocolVersion}");

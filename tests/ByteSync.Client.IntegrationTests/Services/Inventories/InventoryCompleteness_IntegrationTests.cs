...
public class InventoryCompleteness_IntegrationTests : IntegrationTest
{
    ...
    public void Setup()
    {
        ...
        var ctxt = new TestContextGenerator(Container);
        var sessionId = ctxt.GenerateSession();
        var currentEndpoint = ctxt.GenerateCurrentEndpoint();
        ...
        var mockEnvironment = Container.Resolve<Mock<ByteSync.Interfaces.Controls.Applications.IEnvironmentService>>();
        mockEnvironment.Setup(m => m.AssemblyFullName).Returns(IOUtils.Combine(_testDirectoryService.TestDirectory.FullName, "Assembly", "ByteSync.exe"));

        var mockLocalAppData = Container.Resolve<Mock<ILocalApplicationDataManager>>();
        mockLocalAppData.Setup(m => m.ApplicationDataPath).Returns(IOUtils.Combine(_testDirectoryService.TestDirectory.FullName, "AppData"));
    ...
    public async Task Should_wait_for_one_full_inventory_per_data_node()
    {
        ...
        _dataNodeRepository.AddOrUpdate(new[]
        {
            Node("nAa", c1, "Aa"),
            Node("nBa", c2, "Ba"),
            Node("nBb", c2, "Bb"),
        ...
        await _inventoryService.SetLocalInventory(step1, ByteSync.Business.LocalInventoryModes.Full);
        ...
        await _inventoryService.SetLocalInventory(step2, ByteSync.Business.LocalInventoryModes.Full);
    ...
    public async Task Case_insensitive_code_prefix_and_client_match_are_enforced()
    {
        ...
        _dataNodeRepository.AddOrUpdate(new[] { Node("nBb", "CII_B", "Bb") });

        bool? full = null;
        using var sub = _inventoryService.InventoryProcessData.AreFullInventoriesComplete.Subscribe(v => full = v);

        // wrong client
        await _inventoryService.SetLocalInventory(new[] { FullInventory(sessionId, "CII_X", "Bb") }, ByteSync.Business.LocalInventoryModes.Full);
        full.Should().BeFalse();
        ...
        await _inventoryService.SetLocalInventory(new[] { new InventoryFile(wrong, "wrong.zip") }, ByteSync.Business.LocalInventoryModes.Full);
        full.Should().BeFalse();

        // lowercase prefix should still match
        await _inventoryService.SetLocalInventory(new[] { FullInventory(sessionId, "CII_B", "bb") }, ByteSync.Business.LocalInventoryModes.Full);
        full.Should().BeTrue();
    }
    ...
    public async Task No_data_nodes_trivially_reports_full_true()
    {
        ...
        await _inventoryService.SetLocalInventory(Array.Empty<InventoryFile>(), ByteSync.Business.LocalInventoryModes.Full);
        full.Should().BeTrue();
    }
}

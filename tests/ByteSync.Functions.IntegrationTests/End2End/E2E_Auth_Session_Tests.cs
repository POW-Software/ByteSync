...
public class E2E_Auth_Session_Tests
{
    ...
    public async Task Auth_Login_Then_CreateCloudSession_ShouldSucceed()
    {
        ...
        var tokenA = await _api.LoginAsync(loginA);
        ...
        var tokenB = await _api.LoginAsync(loginB);
        ...
        var askResult = await _api.PostJsonAsync<JoinSessionResult>($"session/{sessionId}/askPasswordExchangeKey", askParams, tokenB.AuthenticationTokens!.JwtToken);
        ...
        await _api.PostJsonAsync<object>($"session/{sessionId}/validateJoin", validateParams, tokenB.AuthenticationTokens!.JwtToken);
    }

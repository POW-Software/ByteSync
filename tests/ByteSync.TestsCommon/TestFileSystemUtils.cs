...
public static class TestFileSystemUtils
{
    ...
    public static FileInfo CreateSubTestFile(DirectoryInfo rootDirectory, string relativeFilePath, string content,
        DateTime? lastWriteTime = null, DateTime? lastWriteTimeUtc = null)
    {
        relativeFilePath = relativeFilePath.Replace("/", Path.DirectorySeparatorChar.ToString());
        
        var fileFullName = IOUtils.Combine(rootDirectory.FullName, relativeFilePath);
    ...
    public static DirectoryInfo CreateSubTestDirectory(DirectoryInfo rootDirectory, string relativeDirectoryPath)
    {
        ...
        var directoryFullName = IOUtils.Combine(rootDirectory.FullName, relativeDirectoryPath);

...
public class RemoveDataSourceCommandHandlerTests
{
    ...
    [Test]
    public async Task RemoveDataSource_WhenSuccessful_CallsInvokeClientsService()
    {
        // Arrange
        var sessionId = "testSession";
        var client = new Client { ClientId = "client1", ClientInstanceId = "clientInstanceId1" };
        ...
        A.CallTo(() => _mockInventoryMemberService.GetOrCreateInventoryMember(A<InventoryEntity>.Ignored, sessionId, client))
            .Returns(inventoryMember);

        A.CallTo(() => _mockInvokeClientsService.SessionGroupExcept(sessionId, client)).Returns(_mockByteSyncPush);
        A.CallTo(() => _mockByteSyncPush.DataSourceRemoved(A<DataSourceDTO>.Ignored)).Returns(Task.CompletedTask);

        var request = new RemoveDataSourceRequest(sessionId, client, client.ClientInstanceId, "dataNodeId", encryptedDataSource);

        // Act
        var result = await _removeDataSourceCommandHandler.Handle(request, CancellationToken.None);

        // Assert
        result.Should().BeTrue();
        A.CallTo(() => _mockInvokeClientsService.SessionGroupExcept(sessionId, client)).MustHaveHappenedOnceExactly();
        A.CallTo(() => _mockByteSyncPush.DataSourceRemoved(A<DataSourceDTO>.That.Matches(dto => 
            dto.SessionId == sessionId && 
            dto.ClientInstanceId == client.ClientInstanceId && 
            dto.EncryptedDataSource.Id == encryptedDataSource.Id)))

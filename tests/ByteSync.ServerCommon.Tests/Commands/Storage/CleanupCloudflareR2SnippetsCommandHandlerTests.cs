...
{
    private ICloudflareR2BucketService _cloudflareR2BucketService = null!;
    ...
    public void Setup()
    {
        _cloudflareR2BucketService = A.Fake<ICloudflareR2BucketService>();
        ...
        _handler = new CleanupCloudflareR2SnippetsCommandHandler(_cloudflareR2BucketService, _options, _logger);
    }
    ...
    public async Task Handle_DeletesObjectsOlderThanRetention()
    {
        ...
        A.CallTo(() => _cloudflareR2BucketService.ListObjectsAsync(A<ListObjectsV2Request>._, A<CancellationToken>._))
        ...
        // Assert
        result.Should().Be(2);
        A.CallTo(() => _cloudflareR2BucketService.DeleteObjectAsync(A<DeleteObjectRequest>.That.Matches(r => r.Key == "old1"), A<CancellationToken>._)).MustHaveHappenedOnceExactly();
        A.CallTo(() => _cloudflareR2BucketService.DeleteObjectAsync(A<DeleteObjectRequest>.That.Matches(r => r.Key == "old2"), A<CancellationToken>._)).MustHaveHappenedOnceExactly();
        A.CallTo(() => _cloudflareR2BucketService.DeleteObjectAsync(A<DeleteObjectRequest>.That.Matches(r => r.Key == "recent"), A<CancellationToken>._)).MustNotHaveHappened();
    }
    ...
    public async Task Handle_DoesNothingIfRetentionIsTooLow()
    {
        ...
        // Assert
        result.Should().Be(0);
        A.CallTo(() => _cloudflareR2BucketService.ListObjectsAsync(A<ListObjectsV2Request>._, A<CancellationToken>._)).MustNotHaveHappened();
    }
    ...
    public async Task Handle_ReturnsZeroIfNoObjectsToDelete()
    {
        ...
        // Assert
        result.Should().Be(0);
        A.CallTo(() => _cloudflareR2BucketService.DeleteObjectAsync(A<DeleteObjectRequest>._, A<CancellationToken>._)).MustNotHaveHappened();
    }

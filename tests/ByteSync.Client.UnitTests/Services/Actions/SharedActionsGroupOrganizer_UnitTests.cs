...
public class SharedActionsGroupOrganizer_UnitTests
{
    ...
    public async Task OrganizeSharedActionsGroups_ShouldHandleEmptyList()
    {
        ...
        // Assert
        _sharedActionsGroupRepository.Verify(x => x.SetOrganizedSharedActionsGroups(It.Is<List<SharedActionsGroup>>(s => s.Count == 0)),
            Times.Once);
    }
    ...
    [TestCase("CID1", 1)]
    [TestCase("CID2", 0)]
    public async Task OrganizeSharedActionsGroups_WhenSynchronizeContentAndDate_ShouldSortCorrectly(string clientInstanceId,
        int expectedCount)
    {
        // Arrange

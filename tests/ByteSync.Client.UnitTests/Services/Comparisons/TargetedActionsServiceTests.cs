...
public class TargetedActionsServiceTests
{
    ...
    public void AddTargetedAction_DoNothing_ShouldRemoveExistingAndAddDoNothing()
    {
        ...
        // Act
        _targetedActionsService.AddTargetedAction(doNothingAction,
            new ComparisonItem(new PathIdentity(FileSystemTypes.File, "key", "name", "data")));
        
        // Assert
        _atomicActionRepositoryMock.Verify(r => r.Remove(existingActions), Times.Once);
        _atomicActionRepositoryMock.Verify(r => r.AddOrUpdate(It.Is<AtomicAction>(a => a.Operator == ActionOperatorTypes.DoNothing)),
            Times.Once);
    }
    ...
    public void AddTargetedAction_WhenExistingIsDoNothing_ShouldRemoveExistingAndAddDoNothing()
    {
        ...
        // Act
        _targetedActionsService.AddTargetedAction(newDoNothingAction,
            new ComparisonItem(new PathIdentity(FileSystemTypes.File, "key", "name", "data")));

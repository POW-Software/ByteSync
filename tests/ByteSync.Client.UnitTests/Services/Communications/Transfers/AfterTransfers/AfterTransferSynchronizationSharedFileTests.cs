...
namespace ByteSync.Client.UnitTests.Services.Communications.Transfers.AfterTransfers;
...
public class AfterTransferSynchronizationSharedFileTests
{
    ...
    public async Task OnFilePartUploaded_WhenSynchronizationDataAlreadyTransmitted_ShouldCompleteImmediately()
    {
        ...
        _mockSynchronizationApiClient.Verify(x => x.InformSynchronizationActionError(It.IsAny<SharedFileDefinition>(), It.IsAny<string?>()),
            Times.Never);
    }
    ...
    public async Task OnFilePartUploadedError_WhenSynchronizationDataAlreadyTransmitted_ShouldCompleteAndInformError()
    {
        ...
        task.IsCompletedSuccessfully.Should().BeTrue();
        _mockSynchronizationApiClient.Verify(x => x.InformSynchronizationActionError(_testSharedFileDefinition, It.IsAny<string?>()),
            Times.Once);
    }
    ...
    public void OnFilePartUploadedError_WhenCancellationRequested_ShouldThrowOperationCanceledException()
    {
        ...
        // Act & Assert
        Func<Task> act = async () =>
            await _afterTransferSynchronizationSharedFile.OnFilePartUploadedError(_testSharedFileDefinition, testException);
        act.Should().ThrowAsync<OperationCanceledException>();
    }
    ...
    public void OnUploadFinishedError_WhenCancellationRequested_ShouldThrowOperationCanceledException()
    {
        ...
        // Act & Assert
        Func<Task> act = async () =>
            await _afterTransferSynchronizationSharedFile.OnUploadFinishedError(_testSharedFileDefinition, testException);
        act.Should().ThrowAsync<OperationCanceledException>();
    }
